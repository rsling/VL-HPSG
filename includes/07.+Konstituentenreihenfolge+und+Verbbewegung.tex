...
\begin{frame}
  ...
  \onslide<+->
  \onslide<+->
  Das Verb steht (im Nebensatz) immer \alert{rechts}. Der Rest macht, was er darf.\\
  \onslide<+->
  \Viertelzeile
  \begin{exe}
...
\begin{frame}
  ...
  \onslide<+->
  \onslide<+->
  Sie kennen es aus CP\slash IP-Ansätzen in der GB-Theorie.\\
  \grau{Über die Adäquatheit der Grundidee ist man sich auch weitgehend einig.}\\
  \onslide<+->
  \Halbzeile
  \begin{exe}
  ...
  \scalebox{0.44}{\begin{avm}
    \@3 \[ \asort{word}
      phon & \phon{besucht} \\
        loc & \@1 \[cat & \[
          head|initial & $-$ \\
          subcat & \<\@2 NP\UpSub{\sl Nom}{\alert{\@{10}}}, \@6 NP\UpSub{\sl Acc}{\alert{\@{11}}}\>
        \] \\
        \alert{cont} & \alert{\@7 \[ restr & \< \[ \asort{visit-rel} agent & \@{10} \\ other & \@{11} \] \> \]} \\
      \]
    \]
  \end{avm}}
}
