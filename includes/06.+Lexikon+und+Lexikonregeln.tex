    ...
    \scalebox{0.8}{\begin{avm}
      \[
        ...
        cat & \[
          ...
          subcat & \< \[ cat|head & det \] \> \\
        \] \\
        cont|restr & \[ \asort{sortal-psoa-rel} psoa-arg & \@3 \] \\
        single-dtr & \[ 
          phon & \@2 \\
          cat & \[ 
            head & \@1 \\
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item Theorien wie HPSG sind \alert{Theorien des gesamten Sprachsystems} inkl.\ Lexikon,\\
      sie sind \rot{keine Syntaxen im engeren Sinn}.
  \end{itemize}
\end{frame}

\begin{frame}
  {Passiv mit Morpholomagie}
  \onslide<+->
  \onslide<+->
...
\begin{frame}
  {Plural von Nomina}
  Was leistet Pluralbildung bei Nomina? \onslide<+-> -- Form, Numerus. Ungefähr so:\\
  \onslide<+->
  \Zeile
  \centering 
  \scalebox{0.7}{%
  ...
  \begin{itemize}[<+->]
    ...
    \item Die eigentliche Quantifikation macht der Quantor.
    \item Der Quantor muss aber ein pluralischer sein (\textit{zwei}, \textit{mehrere}, \ldots).\\
      \grau{Das wurde hier behelfsmäßig mit \textsc{nform} implementiert.}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \onslide<+->
  \onslide<+->
  Was leistet Verbflexion? \onslide<+-> -- Form, Tempus, Person, Numerus\grau{, Modus)}\\
  \grau{\footnotesize Vereinfachung für unsere Zwecke von \citet[380]{MuellerLehrbuch3}.}\\
  \onslide<+->
  \Halbzeile
  \centering 
  \scalebox{0.75}{%
...
%   \onslide<+->
%   \onslide<+->
%   Konversion ist letztlich dasselbe wie Deriavation ohne \textsc{phon}-Modifikation.\\
%   \onslide<+->
%   \Zeile
%   \scalebox{0.8}{%
%     \begin{avm}
