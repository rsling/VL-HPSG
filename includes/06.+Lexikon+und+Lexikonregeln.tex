...
\begin{frame}
  ...
  \grau{\footnotesize Wir schreiben jetzt reine Typangaben ohne eckige Klammern.}\\
  \onslide<+->
  \centering 
  \Zeile
  \begin{avm}
...
\begin{frame}
  ...
  \grau{\footnotesize Pronomina führen keine \textsc{restr} ein. Substantive sind dafür immer dritte Person.}\\
  \centering
  \Halbzeile
  \onslide<+->
...
\begin{frame}
  ...
  \scalebox{0.75}{\begin{avm}
    \[
      cat|subcat & \< NP\Sub{nom}, NP\Sub{acc} \> \\
    \]
  \end{avm}}\\
  ...
  \begin{minipage}{0.5\textwidth}
    Im Ergebnis (für Verben wie \textit{jemanden ängstigen}):\\
    ...
    \item In der \alert{Formalisierung} verschwinden diese Einschränkungen:
      \begin{itemize}[<+->]
        ...
        \item Wörter bringen zusätzlich \alert{idiosynkratische} Informationen mit.\\
          \grau{\footnotesize\textit{Frau} ist ein \textit{noun-sign}, \textit{det-sc}, \textit{nominal-sem-sign\slash 3} mit \textit{Frau} als \textsc{phon}-Wert.}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \item Der Quantor muss aber ein pluralischer (\textit{zwei}, \textit{mehrere}, \ldots) sein.\\
      \grau{Das wurde hier behelfsmäßig mit \textsc{numform} implementiert. Besser über \textsc{index}.}
  \end{itemize}
\end{frame}
